...
use crate::service::clipboard::{ArboardBackend, ClipboardService};
use crate::tui::context::TuiContext;
use crate::tui::views::entry_form::{EntryForm, FormAction, FormMode};
use crate::tui::views::main_view::{MainView, MainViewAction};
...
impl Tui {
    ...
    pub async fn new(app: App, context: TuiContext) -> crate::error::Result<Self> {
        // Load vaults
        let vaults = app.registry.list_vaults().await?;

        // Load entries from every vault
        let mut all_entries: Vec<crate::domain::Entry> = Vec::new();
        for vault in &vaults {
        ...
        match &self.active {
            ...
            ActiveView::Main => {
                if self.main_view.is_leader_mode() {
                    ...
                    use crate::tui::views::main_view::Column;
